...
const Shorts = () => {
  ...
  const fetchShorts = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/videos?type=shorts&page=${page}&limit=24`);
  ...
  return (
    <Layout>
      <Section>
        <Container>
          <SectionTitle>{t('shorts')}</SectionTitle>
          ...
          {shorts.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: '2rem' }}>
              <p style={{ color: '#666', fontSize: '1.1rem' }}>
                No shorts available. Shorts will appear here once fetched from YouTube.
              </p>
            </div>
          )}
        </Container>
